...
interface TerminalToggleEditProps {
  dockerImages: DockerImageConfig[];
  currentDockerImage: string | null;
}

export default function TerminalToggleEdit({
  dockerImages,
  currentDockerImage,
}: TerminalToggleEditProps) {
  const [requiresTerminal, setRequiresTerminal] = useState(!!currentDockerImage);
  const [selectedImage, setSelectedImage] = useState(currentDockerImage || '');
  const [validationStatus, setValidationStatus] = useState<
    'idle' | 'validating' | 'valid' | 'invalid'
  >(currentDockerImage ? 'valid' : 'idle');
