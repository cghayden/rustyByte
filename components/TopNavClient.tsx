      ...
      <header className="border-b border-stone-300 bg-stone-900 backdrop-blur sticky top-0 z-50">
        <nav className="px-4 md:px-6 lg:px-8 py-3 flex items-center gap-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {/* Pending review link for ADMIN only */}
            {user && user.role === 'ADMIN' && (
              <Link
                href="/admin/pending"
                className="px-3 py-1.5 rounded-xl text-sm transition-colors bg-yellow-600 text-white hover:bg-yellow-500 font-medium"
                title="Pending Challenge Reviews"
              >
                Pending
              </Link>
            )}
