    ...
    <>
      ...
      <div className="fixed top-0 left-0 h-full w-72 bg-tavern-medium z-50 md:hidden overflow-y-auto shadow-xl border-r border-accent/20">
        <div className="p-4">
          ...
          {user && (
            <div className="mb-6">
              <h3 className="text-xs uppercase tracking-wide text-accent-dark mb-3">Challenges</h3>
              <div className="space-y-2">
                {categories.map((category) => {
                  const categoryChalls = challenges.filter((c) => c.categoryId === category.id);
                      ...
                      {isExpanded && categoryChalls.length > 0 && (
                        <div className="ml-4 mt-1 space-y-1">
                          {categoryChalls.map((challenge) => (
                            <button
                              key={challenge.id}
                              onClick={() => handleNavigation(`/${category.id}/${challenge.slug}`)}
              ...
              <>
                ...
                {user.role === 'ADMIN' && (
                  <button
                    ...
                    className="w-full px-4 py-2 rounded-lg bg-yellow-600 text-white hover:bg-yellow-500 transition-colors font-medium text-sm"
                  >
                    Pending Reviews
                  </button>
                )}
